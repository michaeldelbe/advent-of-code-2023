...
public class Day05part2 {
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    List<Pair<Long, Long>> seedRanges = mapSeedRanges(seeds);

    List<Pair<Thread, Runner>> threads = new ArrayList<>();

    for (Pair<Long, Long> seedRange : seedRanges) {
      Runner runner = new Runner(mappings, seedRange);
      Thread thread = new Thread(runner);
      thread.start();
      threads.add(Pair.of(thread, runner));
    }

    for (Pair<Thread, Runner> thread : threads) {
      thread.getLeft().join();
      results.add(thread.getRight().getResult());
    }
  ...
  private static Long processRange(
      Map<String, List<Mapping>> mappings, Pair<Long, Long> seedRange) {
    long result = Long.MAX_VALUE;
    long seed = seedRange.getLeft();
    long size = seedRange.getLeft() + seedRange.getRight();
    while (seed < size) {
      Long location = findCorrespondingLocation(seed, mappings);
      if (location < result) {
        result = location;
      }
      seed++;
  ...
  private static List<Pair<Long, Long>> mapSeedRanges(String[] seeds) {
    List<Pair<Long, Long>> seedRanges = new ArrayList<>();
    Long seedSource = null;
    for (int i = 0; i < seeds.length; i++) {
      if (i % 2 == 0) {
        seedSource = Long.parseLong(seeds[i]);
      } else {
        seedRanges.add(Pair.of(seedSource, Long.parseLong(seeds[i])));
      }
    }
    return seedRanges;
  }
  ...
  public static class Runner implements Runnable {
    ...
    private final Map<String, List<Mapping>> mappings;
    private final Pair<Long, Long> seedRange;

    public Runner(Map<String, List<Mapping>> mappings, Pair<Long, Long> seedRange) {
    ...
    @Override
    public void run() {
      System.out.println("Processing range: " + seedRange);
      this.result = processRange(mappings, seedRange);
      System.out.println("Finished processing range: " + seedRange + ". Result: " + this.result);
    }
